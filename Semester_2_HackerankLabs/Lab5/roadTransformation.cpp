...
int kruskals(int g_nodes,int g_edges, vector<int> g_from, vector<int> g_to, vector<int> g_weight,vector<int> &MST) {
...
int main()
{
    ...
    string g_nodes_edges_budget_temp;
    getline(cin, g_nodes_edges_budget_temp);

    vector<string> g_nodes_edges_budget = split(rtrim(g_nodes_edges_budget_temp));

    int g_nodes = stoi(g_nodes_edges_budget[0]);
    int g_edges = stoi(g_nodes_edges_budget[1]);
    int g_budget= stoi(g_nodes_edges_budget[2]);
    ...
    if((g_budget<g_nodes-1)or(g_nodes-1>g_edges)){
        fout<<-1;
    }
    else{
        vector<int> MST;

        int res = kruskals(g_nodes, g_edges, g_from, g_to, g_weight,MST);
        if(MST.size()!=g_nodes-1){
            fout<<-1;
        }
        else if (res<=g_budget){
            fout<<0;
        }
        else{
            int iteration=0;
            for(int j=g_nodes-1;j>0;j--){
                if(MST[j-1]!=1){
                    iteration++;
                    if(res-MST[j-1]+1<=g_budget){
                        break;
                    }
                    else{
                        res=res-MST[j-1]+1;
